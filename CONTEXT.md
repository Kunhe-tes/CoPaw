...
The explicit transition from an owning Chat into a W+ SOP Workspace. Both a direct user invocation of `wplus-sop-miner` and an implicitly detected invocation first render a clickable Chat confirmation card. Confirm idempotently creates the persisted W+ SOP Clarification Session in `GeneratingStageProposal`, then uses the already-submitted original request as the Miner's initial input and navigates to the workspace. The Miner's first output is only a validated 2–4-stage proposal; it cannot produce clarification questions until the user confirms or adjusts that queue. Reject is available for implicit detection, creates no SOP Session, keeps the user in normal Chat, and returns the original request to the normal Chat agent exactly once. The rejected turn must suppress `wplus-sop-miner` re-detection so the confirmation card cannot loop.
