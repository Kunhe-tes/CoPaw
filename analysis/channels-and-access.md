...
AgentScope Chat 的工具调用先按 `tool_call_id` 合并调用与结果，再按后端校验后的显式操作组 ID 聚合。首个分组工具进入流时即显示默认折叠的操作组；展开后按原始顺序呈现 reasoning 与规范化工具动作，不展示原始命令、参数或完整输出。响应结束后，操作组整体进入既有“执行过程”折叠；缺少操作组声明的历史事件继续使用原有独立工具卡片。
