...
| Runtime Diagnostic | `src/swe/app/runtime_diagnostic.py`, `src/swe/app/middleware/sse_diagnostic.py`, `src/swe/app/_app.py` | 按 Pod 输出 `RUNTIME_DIAGNOSTIC ` 结构化日志，采集 SSE 并发、事件循环延迟、当前 Python 进程资源和 `/opt/deployments/app` 文件系统使用情况 |
