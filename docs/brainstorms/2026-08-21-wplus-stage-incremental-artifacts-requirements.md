...
- R8. 每次环节确认后，系统必须立即刷新累计 JSON、Markdown 和 HTML，并在工作台提供累计结果预览。
...
- AE4. **Covers R8, R9.** 给定环节 1 已确认且环节 2 尚未确认，当累计预览刷新时，整体文件只包含环节 1 的接受版本，不包含环节 2 的草稿报告。
...
- 每次确认环节后，用户都能立即看到截至当前的累计 SOP，不必等待全部流程结束。
