...
- Market 在首次分发、更新启用技能、删除技能或成功执行 active collision promotion 后触发现有 Agent reload；Market 的变更解析结果显式标识是否发生 promotion：仅同名双副本使登记状态从禁用变为启用时为真，调用方据此决定该通知，不能从目录路径推断。已启用技能的双副本清理不属于 promotion。仅维护禁用技能时不触发 reload。active collision promotion 会将登记技能置为启用，因此会改变 Skill Runtime View。
