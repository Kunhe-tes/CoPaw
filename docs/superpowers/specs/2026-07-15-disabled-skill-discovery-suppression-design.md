...
- Market 在首次分发、更新启用技能、删除技能或成功执行 active collision promotion 后触发现有 Agent reload；Market 的变更解析结果显式标识是否发生 promotion，调用方据此决定该通知，不能从目录路径推断。仅维护禁用技能时不触发 reload。active collision promotion 会将登记技能置为启用，因此会改变 Skill Runtime View。
