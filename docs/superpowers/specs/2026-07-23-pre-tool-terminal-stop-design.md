...
The canonical hook result is `{"decision":"stop","reason":"…"}`. Command and HTTP handlers continue to accept the existing `{"continue":false,"stopReason":"…"}` form. Prompt handlers accept `decision: "stop"` while retaining their strict `{decision, reason}` output shape.
...
For `PostToolUse`, the already successful tool result remains unchanged before the final reason is emitted. For `PostToolUseFailure`, the original tool failure remains available for presentation and audit, but an explicit stop suppresses its propagation into AgentScope. Both post events record hook additional context before the final reason.

`STOP` behavior for hook events other than the three tool-hook events remains unchanged.
...
- `src/swe/agents/tool_guard_mixin.py` distinguishes `hook_stopped` for pre-tool rejection, records post-tool outcomes without rewriting them, and exposes terminal-stop state to the agent turn.
...
Unit tests cover canonical stop normalization for generic and prompt handlers, stop precedence over conflicting updates, `hook_stopped` pre-tool-result creation, and unchanged deny/block behavior. Tool-guard tests cover post-success and post-failure stops, retained tool outcomes, failure suppression, and peer cancellation. Runner tests cover the terminal response, memory persistence, no follow-up model invocation, lifecycle-hook skipping, fallback reason, `suppressOutput` immunity, and ordinary session reuse on the next user turn.
