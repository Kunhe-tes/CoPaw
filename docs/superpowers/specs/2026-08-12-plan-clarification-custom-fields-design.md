...
- 顶层单选、多选保留现有自定义文本行为。
- 表单内每个 `single_choice` 和 `multi_choice` 字段默认显示系统拥有的自定义填写输入框。
...
`allow_custom_response` 在工具输入和 `PlanClarificationCard` 中默认值改为 `true`。模型仍只负责提供实际业务选项；前端生成的自定义填写输入框不是模型选项，不写进 `options`，也没有伪造的业务 ID。

工具说明和 Plan Mode 指令应说明：选择类控件天然支持用户自定义填写，调用工具时无需为“其他”生成候选项。

已有调用显式传入 `allow_custom_response=false` 时保留其语义：不显示字段级自定义填写输入框。这样升级不会改变已有的显式禁用用例。
...
每个 `single_choice` 与 `multi_choice` 字段在业务选项列表下方默认显示自定义填写输入框：

- 单选输入非空时清除标准选项选择；自定义文本非空才满足必填字段。
- 多选保留标准选项和自定义文本并存。若字段必填，至少选择一个标准项或填写自定义文本才满足字段。
- 文本为空不写入提交数据，也不单独满足必填字段。
- 自定义填写输入框固定为单行高度；超长内容可通过横向滚动查看。
- `text` 字段维持现有文本输入，不添加第二个输入框。
