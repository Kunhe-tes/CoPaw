...
def _build_zip_response(
    ...
    # sanitize 文件名（保留中文，只过滤特殊字符）
    safe_name = _sanitize_filename(export_name)
    filename_zip = f"{safe_name}.zip"

    # RFC 6266: 使用 filename* 参数支持 UTF-8 编码
    encoded_filename = quote(filename_zip, safe="")
...
async def export_skill(
    ...
    if source_type == "market":
        ...
        if not item_id:
            raise HTTPException(
                status_code=400,
                detail="item_id required for market type",
            )
        # 获取技能目录
        ...
        if not skill_name:
            raise HTTPException(
                status_code=400,
                detail="skill_name required for my type",
            )
