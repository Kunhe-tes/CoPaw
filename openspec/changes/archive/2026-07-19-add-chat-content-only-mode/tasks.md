...
- [x] 4.6 Verify content-only presentation at `1280x720`, `1440x900`, and `1920x1080` in top-level and embedded use, checking clipping, empty rails, focusability, and horizontal overflow. Top-level and same-origin HTTP iframe checks passed at all three sizes with the title retained, suppressed surfaces absent, the app root filling the container, no residual focusable controls, and no horizontal overflow.
