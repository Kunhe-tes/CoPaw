# console-navigation-visuals Specification
...
The global navigation SHALL use the configured white-first Management Console palette with a white navigation surface, neutral text/icon hierarchy, a light content boundary, and `#3769FC`-based active, hover, and focus emphasis without warm cream, coral, large blue-gray areas, or large saturated-blue areas.
...
- **THEN** the Header and global Sidebar SHALL remain hidden and the embedded content SHALL retain the white-first management canvas without exposed shell gaps
