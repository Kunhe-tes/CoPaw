...
# ── Skill history data initialization ──


class InternalInitSkillHistoryRequest(BaseModel):
    """历史技能数据初始化请求."""

    tenant_id: Optional[str] = Field(
        default=None,
        description="租户 ID，不传时初始化所有租户的技能",
    )
    force: bool = Field(
        default=False,
        description="是否强制重新初始化（覆盖已有 skill_id）",
    )
    dry_run: bool = Field(
        default=False,
...
class InternalInitSkillHistoryResponse(BaseModel):
    """历史技能数据初始化响应."""
    ...
    total_skills: int
    processed: int
    updated_manifest: int
    inserted_db: int
    skipped: int
    errors: list[dict]


async def _init_skill_history_for_tenant(
    tenant_id: str,
    source_id: str,
    force: bool,
    dry_run: bool,
    ...
    try:
        # 获取技能池路径
        skills_dir = SkillsManager.get_skill_pool_dir(tenant_id, source_id)
        if not skills_dir.exists():
            return results

        # 读取 manifest
        manifest_path = skills_dir.parent / "skill.json"
        ...
        # 保存 manifest
        if not dry_run and results["updated_manifest"] > 0:
    ...
    if md_content.startswith("---"):
        ...
        if end_idx != -1:
            ...
            for line in frontmatter.split("\n"):
                if line.startswith("cn_name:") or line.startswith("chinese_name:"):
...
@router.post(
    "/skills/init-history",
    response_model=InternalInitSkillHistoryResponse,
    responses={
        400: {"model": InternalErrorResponse},
        401: {"model": InternalErrorResponse},
        503: {"model": InternalErrorResponse},
...
async def internal_init_skill_history(
    payload: InternalInitSkillHistoryRequest,
    request: Request,
    x_internal_token: Optional[str] = Header(
        default=None,
        alias="X-Internal-Token",
    ),
) -> InternalInitSkillHistoryResponse:
    """初始化历史技能数据，填充 skill_id 和 cn_name.
    ...
    if payload.tenant_id:
        ...
        source_id = ""  # 需要从请求或默认值获取
    else:
        tenant_ids = _list_runtime_tenant_ids()
        source_id = ""

    # 如果没有 source_id，使用默认值
    if not source_id:
        source_id = "default"
    ...
    for tenant_id in tenant_ids:
        result = await _init_skill_history_for_tenant(
            tenant_id,
            source_id,
            payload.force,
            payload.dry_run,
        ...
        updated_manifest += result.get("updated_manifest", 0)
        inserted_db += result.get("inserted_db", 0)
        skipped += result.get("skipped", 0)
        errors.extend(result.get("errors", []))

    return InternalInitSkillHistoryResponse(
        success=len(errors) == 0,
        tenant_id=payload.tenant_id or "all",
