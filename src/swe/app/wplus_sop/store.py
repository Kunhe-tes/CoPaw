...
def _lookup_duplicate_receipt(
    data: WPlusSopStoreFile,
    receipt: CommandReceipt | None,
) -> tuple[SessionRecord, CommandReceipt] | None:
    """Return (record, stored_receipt) if receipt is already indexed; else None."""
    if receipt is None:
        return None
    stored = data.command_index.get(receipt.command_request_id)
    if stored is None:
        return None
    if stored.command != receipt.command:
        raise WPlusSopStoreError(
            "command_request_id is already used by another command",
        )
    session_id = stored.sop_session_id
    if not session_id or session_id not in data.sessions:
        raise WPlusSopStoreError(
            "idempotent Session command has no persisted Session",
        )
    return data.sessions[session_id], stored


def _validate_proposal_scope_for_duplicate(
    data: WPlusSopStoreFile,
    receipt: CommandReceipt,
    expected_proposal_id: str,
    session_id: str,
) -> OwnershipTuple:
    """Validate idempotent receipt belongs to the expected proposal scope."""
    proposal = data.entry_proposals.get(expected_proposal_id)
    proposal_receipt = (
        proposal.command_receipt if proposal is not None else None
    )
    if (
        _proposal_ids_for_command(data, receipt.command_request_id)
        != [expected_proposal_id]
        or proposal is None
        or proposal_receipt is None
        or proposal_receipt.command_request_id != receipt.command_request_id
    ...
    return proposal.ownership


def _validate_session_scope_for_duplicate(
    receipt: CommandReceipt,
    expected_sop_session_id: str,
    actual_session_id: str,
) -> None:
    if actual_session_id != expected_sop_session_id:
        raise WPlusSopStoreError(
            "command_request_id belongs to another Session scope",
        )


def _validate_ownership_for_duplicate(
    record: SessionRecord,
    receipt: CommandReceipt,
    expected_ownership: OwnershipTuple,
) -> None:
    if record.projection.ownership != expected_ownership:
        raise WPlusSopStoreError(
...
class WPlusSopStore:
    ...
    def _find_duplicate(
        ...
        *,
        ...
        found = _lookup_duplicate_receipt(data, receipt)
        if found is None:
            return None
        record, stored_receipt = found
        if expected_proposal_id is not None:
            expected_ownership = _validate_proposal_scope_for_duplicate(
                data,
                receipt,
                expected_proposal_id,
                record.projection.sop_session_id,
            )
        elif expected_sop_session_id is not None:
            _validate_session_scope_for_duplicate(
                receipt,
                expected_sop_session_id,
                record.projection.sop_session_id,
            )
        if expected_ownership is not None:
            _validate_ownership_for_duplicate(
                record,
                receipt,
                expected_ownership,
            )
    ...
    def commit_event(
        ...
        *,
        ...
        with self._lock:
            ...
            if duplicate is not None:
                return duplicate
            idem_result = self._validate_commit_preconditions(
                record,
                sop_session_id,
            ...
            if idem_result is not None:
                return idem_result
            return self._apply_commit_mutation(
                data,
                record,
                event,
                next_state,
    ...
    def _apply_commit_mutation(
        ...
        event: StructuredInteractionEnvelope,
        next_state: SessionState,
        projection_changes: dict[str, Any] | None,
        outbox_item: ChatProjectionOutboxItem | None,
        command_receipt: CommandReceipt | None,
        run_attempt: RunAttempt | None,
        run_completion: tuple[str, str, RunStatus | str] | None,
    ) -> StoreMutation:
        projection = record.projection
        updates = {
        ...
        next_projection = projection.model_copy(update=updates, deep=True)
