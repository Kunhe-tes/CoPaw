...
def test_public_asset_upload_api_route_skips_auth() -> None:
    request = MagicMock()
    request.method = "POST"
    request.url.path = "/api/assets/upload"
    request.client = MagicMock(host="203.0.113.10")
...
def test_health_probe_skips_auth_for_remote_clients() -> None:
    request = MagicMock()
    request.method = "GET"
    request.url.path = "/api/health/health"
    request.client = MagicMock(host="203.0.113.10")
...
def test_runtime_memory_diagnostic_skips_auth_for_remote_clients() -> None:
    request = MagicMock()
    request.method = "GET"
    request.url.path = "/api/runtime/memory-diagnostic"
    request.client = MagicMock(host="203.0.113.10")
