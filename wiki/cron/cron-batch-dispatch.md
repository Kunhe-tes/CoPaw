...
批调度不是后台扫描所有父任务。父批次仍由外部调度平台的物理 timer 触发；Scheduler 的常驻循环只处理已入库 intent 的重试、超时回收、容量调整和补位。
...
外部调度平台
  ...
  -> Scheduler 完成、重试或终止 intent，并立即补位
```
...
| `POST /api/scheduler/cron/execution` | SWE | 回传 dispatch 执行结果，驱动完成、重试和补位 |
...
同一批次内先按阅读热度降序，再按 `due_at`、tenant、job 稳定排序。阅读热度来自最近 30 天成功异步执行的阅读记录，并对 2/3/4/5 小时内快速阅读增加权重；值上限为 9999。源任务本身也参与排序。

容量策略保存在数据库，可按模型作用域配置基线、最小、最大 worker 和调整策略。默认约每 300 秒调整一次：近期有错误时收缩，无错误时逐步增加；一次 execution 完成后的“立即补位”和周期容量调整是两件不同的事。

## 重试与失联回收

- Scheduler 请求 SWE 失败时，intent 按重试策略重新进入 pending。
- SWE 返回失败 execution 时，如果还没达到最大尝试次数，默认 300 秒后重试。
- 默认最多尝试 3 次；达到上限后 intent 标记 failed。
- 已派发但长时间没有 execution 回执的 intent，由 `SCHEDULER_CRON_DISPATCHED_STALE_SECONDS` 判定失联并回收或终止。
...
| `scheduler/src/scheduler/app/services/cron/execution_sync_service.py` | execution 去重、完成、重试和补位 |
...
7. SWE execution meta 是否带完整 `cron_dispatch` 身份，execution 回执是否到达 Scheduler。
