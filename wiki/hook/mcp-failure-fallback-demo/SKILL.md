...
- `Stop` prompt handler 会基于 `assistant_response` 与 HookContext JSON 做收尾前检查；如果候选回复把失败调用说成“已确认成功”，就应该返回 `block`。
